	...
	public function swatch_html( $html, $term, $type, $args ) {
		...
		switch ( $type ) {
			case 'color':
				...
				$html = sprintf(
					'<span class="swatch swatch-color swatch-%s %s" style="background-color:%s;color:%s;" title="%s" data-value="%s">$name</span>',
					esc_attr( $term->slug ),
					$selected,
					esc_attr( $color ),
					"rgba($r,$g,$b,0.5)",
			...
			case 'image':
				...
				$html  = sprintf(
					'<div class="swatch swatch-image swatch-%s %s" title="%s" data-value="%s">
						<div class="group-scents">
