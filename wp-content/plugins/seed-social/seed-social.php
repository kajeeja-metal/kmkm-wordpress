...
function seed_social( $echo = true , $css_class = '') {
	...
	if( $facebook_text == '' ) $facebook_text = '';
	if( $twitter_text == '' ) $twitter_text = '';
	...
	if( $is_facebook || $is_twitter || $is_line ) {
		...
		/* Facebook Button */
		if( $is_facebook )
			$fbshare = '<a href="https://www.facebook.com/share.php?u='.urlencode( get_the_permalink( $post->ID ) ).'" target="seed-social"><i class="ss-facebook"></i></a>';

		/* Twitter Button */
		if( $is_twitter )
			$tweet = '<a href="https://twitter.com/share?url='.urlencode( get_the_permalink( $post->ID ) ).'&text='.urlencode($post->post_title).'" target="seed-social"><i class="ss-twitter"></i></a>';

		/* Line */
		if( $is_line )
			$line = '<a href="https://lineit.line.me/share/ui?url='.urlencode( get_the_permalink( $post->ID ) ).'" target="seed-social"><i class="ss-line"></i></a>';
